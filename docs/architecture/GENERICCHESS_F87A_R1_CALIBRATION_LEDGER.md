# F87A-R2 compact calibration ledger

This tracked ledger is the durable summary of the F87A-R2 RESULT. Raw trajectories and generated JSON remain ignored. The run is bound to PREP baseline `7f1f7b5282f22d50c84d15b488ddf5741e9fad4c`, target `PLAYABILITY`, required layers `A-C`, and the single status reducer. Replay identity is an integrity gate; dynamic playability is a separate qualification gate and remains `DEFER` until calibrated.

| Control | Fingerprint | A / B / C | Key calibration evidence | Terminal counts | Compute |
| --- | --- | --- | --- | --- | --- |
| F86C legacy V4-3 | `7e2ff9e15c2a0d1be5faa8c6697f22e488976a2d2ae9f077b85c6e71f95ff400` | PASS / FAIL / DEFER | `CALIBRATION_NEGATIVE_ENVIRONMENT_AUTHORITY`, `LATTICE_RANK_DEFICIT`, `LATTICE_RESIDUE_CONFINEMENT`, `SINK_COMPONENTS`, `ONE_WAY_TRANSPORT`, `FINITE_REACHABILITY_CONFINEMENT` | stalemate 4; frozen authority stalemate fraction 1.0 | 4 games, 37 plies, 0 search nodes |
| F86I full-reverse V4-3 | `8ca58376a52e539c7c8519e902b8dd9e6991b002586d36d846a7a864fffea05d` | PASS / FAIL / DEFER | `CALIBRATION_NEGATIVE_ENVIRONMENT_AUTHORITY`, `HIGH_LOCAL_REVERSIBILITY`, `TERMINAL_TEMPLATE_TRANSPORT_INSUFFICIENT`, plus finite/lattice diagnostics | CENSORED 4 | 4 games, 128 plies, 0 search nodes |
...
Known negative controls have `measurement_status=PASS`, `calibration_expectation_status=PASS`, and ruleset Layer-B `FAIL` under explicit frozen authority; their reducer `overall_status=FAIL`. Boundary and semantic-positive controls remain `DEFER`. CENSORED trajectories carry null outcome scores and do not enter side-bias denominators. All four legacy/boundary controls use canonical JSON action ordering; opening sensitivity is `UNMEASURED` because the bounded run has one opening identity. Layer D/E, Arena, training, Heavy, C2, F85, and QD/MAP-Elites remain out of scope.
