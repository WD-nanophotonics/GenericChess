...
from scripts import f153_sigma070_gen1_tiebreak_mutant5_weak_opening_diagnostic as diagnostic
...
def _stubbed_run(tmp_path, monkeypatch, raw_games):
    ...
    def play(_compiled, seen_opening, gen0, mutant, owner, _ordering, *, deadline, game_timeout):
...
def test_exact_candidate_and_envelope():
...
def test_classification_compares_against_both_prior_pair_scores():
...
def test_exactly_one_pinned_role_swapped_pair(tmp_path, monkeypatch):
    ...
    assert result["pair"]["difference_vs_mutant_1"] == 0.5
    assert result["pair"]["difference_vs_mutant_4"] == 0.5
...
def test_raw_incomplete_game_is_persisted_before_assessment_and_stops(tmp_path, monkeypatch):
    raw = [_raw_game(0, winner=None, nodes=106_001, plies=129, scored_plies=107,
                     valid=False, completed=False, elapsed=481.0, reason="wall_clock_cap")]
    output, calls = _stubbed_run(tmp_path, monkeypatch, raw)
    ...
    def assess_after_durable_write(game, envelope):
        ...
        assert persisted["actions"] == raw[0]["actions"]
        assert "resource_envelope_compliant" not in persisted
    ...
    assert persisted["resource_bound_violations"] == [
        {"metric": "plies", "observed": 129, "approved_limit": 128},
        {"metric": "scored_plies", "observed": 107, "approved_limit": 106},
        {"metric": "nodes", "observed": 106_001, "approved_limit": 106_000},
        {"metric": "elapsed_seconds", "observed": 481.0, "approved_limit": 480},
    ]
...
def test_incomplete_cli_is_nonzero(tmp_path, monkeypatch):
    monkeypatch.setattr(diagnostic.sys, "argv", ["diagnostic", "--output-dir", str(tmp_path / "out")])
