...
def test_conditional_guards_are_dynamic_and_not_static_value(evidence):
    ...
    assert "guard availability" in row["reason"]
...
def test_redundancy_does_not_select_pawn_nonzero_signals_by_correlation(evidence):
    for row in evidence["redundancy_subsumption"]["pairwise"].values():
        assert row["same_executable_cause"] is False
        assert row["recoverable_from_other"] is False
        assert row["real_rule_contrasts_are_genuinely_different"] is True
