...
def test_f41_manifest_is_frozen_and_production_scoped():
 ...
 subprocess.run(["git","cat-file","-e",f"{d['baseline']['product_authority']}^{{commit}}"],cwd=ROOT,check=True,capture_output=True)
 for b in d["inputs"].values(): assert len(b["sha256"])==64 and all(c in "0123456789abcdef" for c in b["sha256"])
 assert all(len(sha)==64 and all(c in "0123456789abcdef" for c in sha) for sha in CORRECTED_INPUT_SHAS.values())
