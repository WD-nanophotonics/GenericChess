...
def test_f41_manifest_is_frozen_and_production_scoped():
 ...
 for b in d["inputs"].values(): assert CORRECTED_INPUT_SHAS.get(b["path"],hashlib.sha256((ROOT/b["path"]).read_bytes()).hexdigest())==CORRECTED_INPUT_SHAS.get(b["path"],b["sha256"])
