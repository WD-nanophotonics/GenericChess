...
def build_manifest() -> dict[str, Any]:
    return {
        ...
        "constraints": ["NO_TUNING_FROM_RESULTS=true", "NO_PRODUCTION_CHANGE_IN_H33A", "NO_QSEARCH_SET_REDUCTION", "NO_QDEPTH_CHANGE", "NO_NATIVE_REPAIR", "NO_RULE_SCHEMA_IR_CHANGE", "NO_ALPHASHO_RERUN", "NO_PAIRED_BENCHMARK", "NO_ALPHA_CHESS"],
...
def _run_audit():
    ...
    b_reduction = {str(budget): 1 - classifier["CANDIDATE_B_SEMANTIC_PREVIEW"][str(budget)]["full_pushes"] / classifier["BASELINE"][str(budget)]["full_pushes"] if classifier["BASELINE"][str(budget)]["full_pushes"] else 0.0 for budget in NODE_BUDGETS}
    ...
    root_gain = sum(wall["CANDIDATE_B_SEMANTIC_PREVIEW"]["0.5"][pid]["completed_depth"] > wall["BASELINE"]["0.5"][pid]["completed_depth"] for pid in wall["BASELINE"]["0.5"])
    fallback_gain = sum(wall["BASELINE"]["0.5"][pid]["fallback"] and not wall["CANDIDATE_B_SEMANTIC_PREVIEW"]["0.5"][pid]["fallback"] for pid in wall["BASELINE"]["0.5"])
    timing = {str(budget): {"baseline_seconds": statistics.median(row["elapsed_seconds"] for row in fixed["BASELINE"][str(budget)].values()), "candidate_a_seconds": statistics.median(row["elapsed_seconds"] for row in fixed["CANDIDATE_A_POST_PUSH_GAVE_CHECK"][str(budget)].values()), "candidate_b_seconds": statistics.median(row["elapsed_seconds"] for row in fixed["CANDIDATE_B_SEMANTIC_PREVIEW"][str(budget)].values())} for budget in NODE_BUDGETS}
    timing_gain = {budget: 1 - values["candidate_b_seconds"] / values["baseline_seconds"] if values["baseline_seconds"] else 0.0 for budget, values in timing.items()}
    b_performance = (max(timing_gain.values()) >= 0.20 and min(timing_gain.values()) >= -0.10)
    gates = {"candidate_b_classifier_parity": classifier_parity, "candidate_b_fixed_result_parity": b_parity, "candidate_b_structural_gate": b_structural, "candidate_b_performance_gate": b_performance, "candidate_b_root_accessibility_gate": root_gain >= 3 or fallback_gain >= 3, "candidate_b_retention": classifier_parity and b_parity and b_structural and b_performance and (root_gain >= 3 or fallback_gain >= 3), "candidate_a_classifier_parity": all(classifier["CANDIDATE_A_POST_PUSH_GAVE_CHECK"][str(budget)]["mismatches"] == 0 for budget in NODE_BUDGETS), "candidate_a_fixed_result_parity": all(all(values.values()) for budget in parity.values() for values in budget.values() for values in values.values())}
