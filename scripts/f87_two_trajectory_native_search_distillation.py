"""F87: two-trajectory native-search distillation with one frozen Arena4 probe."""
...
from generic_chess.learning.arena import (
    ArenaConfig, ArenaExecutionCaps, ArenaExecutionError, ArenaOpeningCorpus,
...
WORK_ORDER = "GENERICCHESS_F87_TWO_TRAJECTORY_NATIVE_SEARCH_DISTILLATION"
OUT = ROOT / "artifacts/f87_two_trajectory_native_search_distillation"
...
def _rows(compiled, native, parent, trajectory):
    type_ids = tuple(sorted(parent.board_weights))
    rows = []
    for point in trajectory.points:
        prefix = tuple(trajectory.actions[: point.ply])
    ...
    if not rows:
        raise RuntimeError("F87 requires at least one training root per trajectory")
    return rows


def build() -> dict:
    allocation, compiled, native, parent = _load_parent()
    configs = [SelfPlayConfig(games=1, nodes_per_move=512, max_depth=12, seed=seed, epsilon=0.0, tt_megabytes=8, max_plies=64) for seed in SEEDS]
    trajectories = [collect_self_play(compiled, native, parent, config)[0] for config in configs]
    for trajectory in trajectories:
        if not trajectory.points or (trajectory.truncated and trajectory.bootstrap_value is None):
            raise RuntimeError("F87 trajectory gate failed")
    data = []
    trajectory_rows = []
    for trajectory in trajectories:
        rows = _rows(compiled, native, parent, trajectory)
        trajectory_rows.append({"trajectory_id": trajectory.trajectory_id, "seed": configs[len(trajectory_rows)].seed, "root_count": len(rows), "action_row_count": sum(len(row["keys"]) for row in rows)})
        data.extend(rows)
    ...
    if not (objective_after < objective_before and np.isfinite(objective_after) and np.all(np.isfinite(candidate_residual))):
        raise RuntimeError("F87 pairwise objective did not improve")
    if float(np.max(np.abs(candidate_residual))) > 2.0 * float(np.max(np.abs(parent_residual))) + 1e-9:
        raise RuntimeError("F87 residual 2x safety cap failed")
    if parent_model.hidden_weights == raw_model.hidden_weights and parent_model.hidden_bias == raw_model.hidden_bias and parent_model.output_weights == raw_model.output_weights:
        raise RuntimeError("F87 nonlinear parameters did not change")
    identity = {"work_order": WORK_ORDER, "parent_checkpoint_id": parent.checkpoint_id, "parent_model_sha256": stable_sha256(parent.compact_nonlinear), "selfplay_configs": [asdict(config) for config in configs], "trajectories": trajectory_rows, "optimizer": fit, "objective_before": objective_before, "objective_after": objective_after}
    ...
    descriptor = {"schema": "generic-chess-f87-two-trajectory-descriptor-v2", "work_order": WORK_ORDER, "parent_checkpoint_id": parent.checkpoint_id, "candidate_checkpoint_id": candidate.checkpoint_id, "candidate_checkpoint_ref": {"path": "checkpoints/f86_compact_checkpoint.json", "variant": "candidate"}, "parent_model_sha256": stable_sha256(parent.compact_nonlinear), "candidate_model_sha256": stable_sha256(candidate.compact_nonlinear), "training_config_hash": training_hash, "selfplay_configs": [asdict(config) for config in configs], "trajectories": trajectory_rows, "optimizer": fit, "objective_before": objective_before, "objective_after": objective_after, "parent_max_abs_residual": float(np.max(np.abs(parent_residual))), "candidate_max_abs_residual": float(np.max(np.abs(candidate_residual))), "allocation_sha256": allocation["allocation_sha256"]}
    descriptor["descriptor_sha256"] = stable_sha256(descriptor)
    result = {"schema": "generic-chess-f87-two-trajectory-result-v1", "status": "SUCCESSOR_READY_FOR_APPROVED_ARENA", "work_order": WORK_ORDER, "parent_checkpoint_id": parent.checkpoint_id, "candidate_checkpoint_id": candidate.checkpoint_id, "candidate_descriptor_path": str(DESCRIPTOR.relative_to(ROOT)).replace("\\", "/"), "candidate_descriptor_sha256": descriptor["descriptor_sha256"], "training_config_hash": training_hash, "trajectory_ids": [row["trajectory_id"] for row in trajectory_rows], "trajectory_seeds": list(SEEDS), "training_root_count": sum(row["root_count"] for row in trajectory_rows), "training_action_row_count": sum(row["action_row_count"] for row in trajectory_rows), "objective_before": objective_before, "objective_after": objective_after, "parent_max_abs_residual": descriptor["parent_max_abs_residual"], "candidate_max_abs_residual": descriptor["candidate_max_abs_residual"], "behavior_changed": True}
...
def run_arena4_opening(*, opening_index: int, progress_dir: Path, result_path: Path, stage_wall_seconds: int = 7200) -> dict:
    ...
    candidate.validate_ruleset(compiled)
    registered = ArenaOpeningCorpus.from_dict(allocation["selection_and_strength_corpora"]["Arena4"]["corpus"])
...
if __name__ == "__main__":
    ...
    if args.arena_only:
        if args.progress_dir is None or args.result_path is None:
            parser.error("--arena-only requires paths")
