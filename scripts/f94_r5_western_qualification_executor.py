...
def _validate_prep_identity(payload: dict[str, Any], root: Path) -> None:
    ...
    if not isinstance(protocol_sha, str) or len(protocol_sha) != 40:
        raise RuntimeError("qualification PREP protocol source SHA is invalid")
    if payload.get("source_sandbox_sha") != protocol_sha:
        raise RuntimeError("qualification PREP source/protocol provenance mismatch")
    try:
    ...
    for source_key in ("source_p0_prep_artifact", "source_r5_prep_artifact"):
        source = root / payload[source_key]
        if _sha256(source) != payload[f"{source_key}_sha256"]:
            raise RuntimeError(f"qualification source SHA mismatch: {source_key}")
...
def run_qualification_control(
    *,
    ...
    execution_compiled = compile_ruleset_for_execution(ruleset)
    profile = build_ruleset_profile(execution_compiled, EvaluationConfig())
    ...
    for tape in payload["opening_corpora"]:
        ...
        if opening.opening_seed != tape["selected_opening_seed"] or opening.final_position_key != tape["selected_final_position_key"]:
            raise RuntimeError("qualification opening identity changed")
