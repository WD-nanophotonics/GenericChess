"""F148: threshold-10 Standard Shogi score race with capture reward five."""
...
PILOT_SEED = 1_480_001
DISCRIMINATION_SEED = 1_480_101
SCREENING_SEEDS = {1: 1_481_001, 2: 1_481_002}
PROMOTION_SEEDS = {1: 1_482_001, 2: 1_482_002}
...
def _capture_value(captured, mover: int, compiled) -> int:
    return CAPTURE_POINTS if captured is not None and captured.owner != mover and not compiled.types_by_id[captured.current_type_id].is_anchor else 0


def race_points(capture: int, check: int) -> int:
    return CAPTURE_POINTS * capture + CHECK_POINTS * check
...
def score_event(before_position, action, after_position, mover: int, compiled) -> dict:
    ...
    capture = int(_capture_value(captured, mover, compiled) > 0)
    gave_check = int(is_in_check(after_position, 1 - mover, compiled))
    return {"capture": capture, "check": gave_check, "points": race_points(capture, gave_check)}
...
def _pair_task(payload):
    ...
    return {"pair_index": opening.index, "opening_id": opening.final_position_key, "games": games, "valid": all(game["valid"] for game in games), "pair_score": None if any(score is None for score in scores) else sum(scores) / 2.0}
            ...
            if all(row["valid"] for row in rows):
                ...
                if len(selected) == SCREENING_TARGET_PAIRS:
                    ...
                    for i, mutant in enumerate(mutants):
                        ...
                        records.append({"mutant_index": i, "vector": _vector_record(mutant), "result": summarize({"rows": chosen, "attempts": chosen, "invalid_pairs": 0}, 1_483_000 + i), "rows": chosen})
                    return records, {"shared_opening_ids": selected, "invalid_attempts": invalid, "pool_opening_count": len(pool)}
            else:
                invalid.extend({"mutant_index": i, "pair_index": row["pair_index"], "valid": row["valid"]} for i, row in enumerate(rows) if not row["valid"])
    raise RuntimeError("F148 common screening opening pool exhausted")
...
def run(*, output: Path, workers=4, pilot_only=False):
    ...
    pilot_run = run_pairs(gen0, gen0, _openings(compiled, PILOT_SEED, PILOT_POOL_OPENINGS), ordering_values, workers, PILOT_TARGET_PAIRS)
    pilot = summarize(pilot_run, 1_485_001)
    ...
    base.update({"gen0": {"seed": GEN0_SEED, "vector": _vector_record(gen0), "sanity": _sanity(gen0)}, "pilot": pilot, "pilot_rows": pilot_run["rows"], "pilot_only": pilot_only, "m140_diagnostic": {"seed": 1_440_401, "sigma": 1.40, "vector": _vector_record(m140)}})
    if pilot_only or not pilot["acceptance"]:
        base["classification"] = "SCORE_RACE_CAPTURE5_PILOT_ACCEPTED" if pilot["acceptance"] else "SCORE_RACE_CAPTURE5_PILOT_FAILED_DENSITY"
        ...
        return base
    discr_run = run_pairs(gen0, m140, _openings(compiled, DISCRIMINATION_SEED, DISCRIMINATION_POOL_OPENINGS), ordering_values, workers, DISCRIMINATION_TARGET_PAIRS)
    discr = summarize(discr_run, 1_485_101)
    ...
    base.update({"discrimination": discr, "discrimination_rows": discr_run["rows"]})
    if not discr["acceptance"]:
        base["classification"] = "SCORE_RACE_CAPTURE5_DENSE_BUT_INSUFFICIENT_MATERIAL_DISCRIMINATION"
        base["source_sha256"] = hashlib.sha256(Path(__file__).read_bytes()).hexdigest()
        base["git_sha"] = subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True).strip()
        output.write_text(json.dumps(base, indent=2, sort_keys=True), encoding="utf-8")
        return base
