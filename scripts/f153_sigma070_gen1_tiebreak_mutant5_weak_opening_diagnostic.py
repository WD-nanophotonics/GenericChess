"""One-opening paired diagnostic for the tied sigma-.70 Gen1 mutant_5."""
...
BASE_SHA = "f303b91d2c4b08115a83f5323d054eeca86d3376"
OUTPUT_DIR = ROOT / ".generic_chess_flow/f153-sigma070-gen1-mutant5-weak-opening-output"
SCHEMA = "F153_SIGMA070_GEN1_TIEBREAK_MUTANT5_WEAK_OPENING_DIAGNOSTIC_V1"
GEN0_VALUES = (250, 634, 1000, 3195, 433, 1155, 3613, 658, 1351, 1822, 2299, 856, 240)
MUTANT_INDEX = 5
MUTANT_VALUES = (307, 175, 798, 1526, 173, 1000, 4495, 321, 3281, 2754, 2718, 2292, 394)
MUTANT_SEQUENCE_SHA256 = "34412f0ec465578cdb3578fecf1e3381e893573e6807c8855a3d3ae41d29819c"
OPENING = {
    "seed": 1_590_402,
...
TT_ENTRIES = 250_000
CLASSIFICATIONS = {
    "improves": "GEN1_TIEBREAK_MUTANT5_WEAK_OPENING_IMPROVES",
    "equals": "GEN1_TIEBREAK_MUTANT5_WEAK_OPENING_EQUALS",
    "not_improved": "GEN1_TIEBREAK_MUTANT5_WEAK_OPENING_NOT_IMPROVED",
    "inconclusive": "GEN1_TIEBREAK_MUTANT5_WEAK_OPENING_INCONCLUSIVE",
}
...
def resource_envelope() -> dict:
    return {
        ...
        "envelope_id": "f153-sigma070-gen1-mutant5-weak-opening-v1",
        "logical_cpu_count": 2,
        ...
        "purpose": (
            "One role-swapped mutant_5 versus Gen0 pair on the already-pinned seed-1590402 opening, "
...
def _candidate() -> tuple[tuple[int, ...], tuple[int, ...]]:
    ...
    if len(vectors) <= MUTANT_INDEX or vectors[MUTANT_INDEX] != MUTANT_VALUES:
        raise AssertionError("mutant_5 differs from the amended Chat order")
    if mutations._sha(vectors[MUTANT_INDEX]) != MUTANT_SEQUENCE_SHA256:
        raise AssertionError("mutant_5 sequence SHA256 differs")
...
def _classify(score: float | None) -> str:
    ...
    if score > PRIOR_MUTANT1_PAIR_SCORE:
...
def _pair_record(games: list[dict]) -> dict:
    ...
    return {
        ...
        "prior_mutant_4_pair_score": PRIOR_MUTANT4_PAIR_SCORE,
        "difference_vs_mutant_1": None if score is None else score - PRIOR_MUTANT1_PAIR_SCORE,
        "difference_vs_mutant_4": None if score is None else score - PRIOR_MUTANT4_PAIR_SCORE,
        "aggregate_mutant_minus_gen0_event_points": sum(
            game["candidate_minus_gen0_event_points"] for game in games
        ),
        "games": games,
    }
...
def _base_result(head: str, envelope: dict) -> dict:
    return {
        ...
        "git_sha": head,
        "unknown": "whether tied mutant_5 scores above mutant_1 on the pinned weakest promotion opening",
        "candidate": {
            "name": "mutant_5",
            "index": MUTANT_INDEX,
        ...
        "opening": {**OPENING, "index": 0, "reused_existing_pinned_opening": True,
                    "replacement_allowed": False},
        "prior_pair_scores_on_opening": {"mutant_1": 0.25, "mutant_4": 0.25},
        "score_race": {
            "capture_points": 1,
            "check_points": 1,
            ...
            "pair_score": "mean of mutant_5 game scores for candidate owners 0 and 1",
        },
        "search": {
            "material_only": True,
...
def run(*, output_dir: Path) -> dict:
    ...
    try:
        ...
        result["incomplete_reason"] = f"pinned_opening_or_compile_unusable:{type(exc).__name__}:{exc}"
        _atomic_json(output_dir / "result.json", result)
        return result
    ...
    games = []
    stop_reason = None
    previous_node_ceiling = bounded_game.MAX_TOTAL_NODES
    bounded_game.MAX_TOTAL_NODES = MAX_NODES_PER_GAME * MAX_GAMES
    try:
        for owner in ROLE_ORDER:
            if time.monotonic() >= deadline:
                stop_reason = f"owner_{owner}_internal_wall_cap_before_start"
                break
            ...
            game_path = output_dir / f"seed-{OPENING['seed']}-candidate-owner-{owner}.json"
            # Preserve the exact returned game before adding fields or checking resource bounds.
            _atomic_json(game_path, raw)
            ...
            if stop_reason:
                stop_reason = f"owner_{owner}_{stop_reason}"
            pair = _pair_record(games) if len(games) == MAX_GAMES else {
                "valid": False, "pair_score": None, "games": games,
            }
            result.update({
                "games": games,
        ...
        bounded_game.MAX_TOTAL_NODES = previous_node_ceiling

    pair = _pair_record(games) if len(games) == MAX_GAMES else {
        "valid": False, "pair_score": None, "games": games,
    }
    score = pair["pair_score"]
    nodes = sum(game.get("nodes", 0) for game in games)
    violations = []
    if len(games) > MAX_GAMES:
        violations.append({"metric": "games_attempted", "observed": len(games), "approved_limit": MAX_GAMES})
    if nodes > MAX_TOTAL_NODES:
        violations.append({"metric": "total_nodes", "observed": nodes, "approved_limit": MAX_TOTAL_NODES})
    if violations:
        score = None
    ...
    result.update({
        ...
        "resource_bound_violations": violations,
        "incomplete_reason": stop_reason if score is None else None,
...
def main() -> int:
    ...
    print(json.dumps({
        ...
        "difference_vs_mutant_1": result.get("pair", {}).get("difference_vs_mutant_1"),
        "difference_vs_mutant_4": result.get("pair", {}).get("difference_vs_mutant_4"),
    }, sort_keys=True))
