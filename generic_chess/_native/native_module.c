...
#define GC_SEM_ENGINE_CAPSULE "generic_chess._native_core.gc_semantic_engine"
#define GC_SEM_TT_CAPSULE "generic_chess._native_core.gc_semantic_tt"
#define GC_SEM_DECL_ACTION_TAG UINT64_C(0x8000000000000000)
...
typedef struct {
    ...
    PyObject *ordering_dynamic_values;
    PyObject *ordering_spatial_values;
    PyObject *ordering_localized_control_values;
...
static void gc_semantic_engine_capsule_free(PyObject *capsule) {
    GCSemanticSearchEngine *engine = (GCSemanticSearchEngine *)PyCapsule_GetPointer(
    ...
    if (engine != NULL) {
        ...
        Py_XDECREF(engine->ordering_spatial_values);
        Py_XDECREF(engine->ordering_localized_control_values);
        Py_XDECREF(engine->ordering_compact_values);
        free(engine);
    }
}
...
static int gc_semantic_set_u64(PyObject *mapping, const char *name,
    ...
    int status = PyDict_SetItemString(mapping, name, object);
    Py_DECREF(object);
    return status;
}

static PyObject *gc_semantic_u64_tuple(const uint64_t *values, size_t count) {
...
static int gc_semantic_probe_material(
    ...
    for (uint16_t sq = 0; sq < rules->board_size * rules->board_size; sq++) {
        ...
        if (!piece->occupied) continue;
        int64_t value = profile && profile->supplied ? profile->board[piece->current_type] : (int)piece->current_type + 1;
        score += piece->owner == position->side_to_move ? value : -value;
        if (profile && profile->spatial_supplied) {
            uint16_t rank = sq / rules->board_size;
            uint16_t file = sq % rules->board_size;
            uint16_t cell_rank = (uint16_t)((rank * 3) / rules->board_size);
            uint16_t cell_file = (uint16_t)((file * 3) / rules->board_size);
            if (cell_rank > 2) cell_rank = 2;
            if (cell_file > 2) cell_file = 2;
            int64_t spatial = profile->spatial[piece->owner][piece->current_type][cell_rank * 3 + cell_file];
            score += (position->side_to_move == 0 ? 1 : -1) *
                (piece->owner == 0 ? spatial : -spatial);
        }
    }
    for (uint8_t owner = 0; owner < 2; owner++) {
        for (uint16_t type = 0; type < rules->type_count; type++) {
            int64_t value = profile && profile->supplied ? profile->hand[type] : (int)type + 1;
            int64_t hand_score = (int)position->hand_counts[owner][type] * value;
    ...
    if (profile && profile->localized_control_supplied) {
        int features[GC_SEMANTIC_SPATIAL_CELLS] = {0};
        /* Local control is residualized against each owner's total action
         * count, so its nine cells do not duplicate global mobility. */
        int counts[2][GC_SEMANTIC_SPATIAL_CELLS] = {{0}};
        int totals[2] = {0, 0};
        for (uint8_t owner = 0; owner < 2; owner++) {
            ...
            for (size_t i = 0; i < actions.count; i++) {
                ...
                uint16_t target = (uint16_t)(raw & 0xFFu);
                uint16_t rank = target / rules->board_size;
                uint16_t file = target % rules->board_size;
                uint16_t cell_rank = (uint16_t)((rank * 3) / rules->board_size);
                uint16_t cell_file = (uint16_t)((file * 3) / rules->board_size);
                if (cell_rank > 2) cell_rank = 2;
                if (cell_file > 2) cell_file = 2;
                counts[owner][cell_rank * 3 + cell_file]++;
                totals[owner]++;
            }
            gc_semantic_action_buffer_free(&actions);
        }
        for (int cell = 0; cell < GC_SEMANTIC_SPATIAL_CELLS; cell++) {
            features[cell] = 9 * (counts[0][cell] - counts[1][cell]) -
                (totals[0] - totals[1]);
            score += (position->side_to_move == 0 ? 1 : -1) *
...
static int gc_semantic_spatial_feature_vector(
    ...
    const GCSemanticPosition *position,
    int *occupancy,
    int control[GC_SEMANTIC_SPATIAL_CELLS]) {
    memset(occupancy, 0, sizeof(int) * 2 * rules->type_count * GC_SEMANTIC_SPATIAL_CELLS);
    memset(control, 0, sizeof(int) * GC_SEMANTIC_SPATIAL_CELLS);
    for (uint16_t sq = 0; sq < rules->board_size * rules->board_size; sq++) {
        const GCPiece *piece = &position->board[sq];
        if (!piece->occupied) continue;
        uint16_t rank = sq / rules->board_size;
        ...
        if (cell_rank > 2) cell_rank = 2;
        if (cell_file > 2) cell_file = 2;
        occupancy[(piece->owner * rules->type_count + piece->current_type) * GC_SEMANTIC_SPATIAL_CELLS + cell_rank * 3 + cell_file] +=
            piece->owner == 0 ? 1 : -1;
    }
    int counts[2][GC_SEMANTIC_SPATIAL_CELLS] = {{0}};
    int totals[2] = {0, 0};
    ...
    for (int cell = 0; cell < GC_SEMANTIC_SPATIAL_CELLS; cell++) {
        control[cell] = 9 * (counts[0][cell] - counts[1][cell]) -
            (totals[0] - totals[1]);
    }
    return 1;
}

static PyObject *gc_semantic_spatial_features(PyObject *self, PyObject *args) {
    (void)self;
    PyObject *rules_capsule, *position_capsule;
    if (!PyArg_ParseTuple(args, "OO", &rules_capsule, &position_capsule)) return NULL;
    GCSemanticRules *rules = (GCSemanticRules *)PyCapsule_GetPointer(
    ...
    int *occupancy = (int *)calloc(
        (size_t)2 * rules->type_count * GC_SEMANTIC_SPATIAL_CELLS, sizeof(int));
    if (!occupancy) { PyErr_NoMemory(); return NULL; }
    int control[GC_SEMANTIC_SPATIAL_CELLS];
    if (!gc_semantic_spatial_feature_vector(rules, position, occupancy, control)) {
        free(occupancy);
        PyErr_SetString(PyExc_RuntimeError, "semantic spatial feature extraction failed");
        return NULL;
    }
    PyObject *occupancy_tuple = PyTuple_New(
        (Py_ssize_t)2 * rules->type_count * GC_SEMANTIC_SPATIAL_CELLS);
    PyObject *control_tuple = PyTuple_New(GC_SEMANTIC_SPATIAL_CELLS);
    if (!occupancy_tuple || !control_tuple) {
        Py_XDECREF(occupancy_tuple); Py_XDECREF(control_tuple); free(occupancy);
        return NULL;
    }
    for (Py_ssize_t i = 0; i < (Py_ssize_t)2 * rules->type_count * GC_SEMANTIC_SPATIAL_CELLS; i++)
        PyTuple_SET_ITEM(occupancy_tuple, i, PyLong_FromLong(occupancy[i]));
    for (int i = 0; i < GC_SEMANTIC_SPATIAL_CELLS; i++)
        PyTuple_SET_ITEM(control_tuple, i, PyLong_FromLong(control[i]));
    free(occupancy);
    PyObject *out = Py_BuildValue("{s:O,s:O}",
                                  "occupancy", occupancy_tuple,
                                  "localized_control", control_tuple);
    Py_DECREF(occupancy_tuple);
    Py_DECREF(control_tuple);
    return out;
}
...
typedef struct {
    ...
    uint64_t ordering_evaluations_by_ply[GC_SEM_MAX_PLY + 1];
    uint64_t ordering_nodes_by_ply[GC_SEM_MAX_PLY + 1];
    uint64_t ordering_actions_by_ply[GC_SEM_MAX_PLY + 1];
    uint64_t ordering_elapsed_nanoseconds_by_ply[GC_SEM_MAX_PLY + 1];
    ...
    uint64_t root_first_actions[GC_SEM_MAX_PLY + 1];
    uint8_t root_first_action_present[GC_SEM_MAX_PLY + 1];
...
static uint64_t gc_semantic_declaration_action(uint8_t declaration_index) {
    return GC_SEM_DECL_ACTION_TAG | (uint64_t)declaration_index;
}

static int gc_semantic_is_declaration_action(uint64_t action) {
    return (action & GC_SEM_DECL_ACTION_TAG) != 0;
}

static int gc_semantic_declaration_win(
        const GCSemanticRules *rules, const GCSemanticPosition *position,
        uint32_t ply, uint64_t *action_out) {
    for (uint8_t i = 0; i < rules->declaration_count; i++) {
...
static int gc_semantic_order_actions(GCSemanticIterativeContext *ctx,
    ...
    size_t legal_count = 0;
    for (size_t i = 0; i < actions->count; i++) {
        if (!gc_semantic_iterative_check_budget(ctx, 1)) {
            free(ordered);
            return 0;
        }
        GCSemanticPosition child;
        ...
        if (ctx->ordering_cache != NULL) {
            ...
            if (cache_hit) {
                ...
                ctx->ordering_cache_hits_by_ply[ply]++;
            } else {
                ctx->ordering_cache_misses++;
                ctx->ordering_cache_misses_by_ply[ply]++;
                if (collision) ctx->ordering_cache_collisions++;
                if (collision) ctx->ordering_cache_collisions_by_ply[ply]++;
                int dynamic[3] = {0, 0, 0};
                ...
                ctx->ordering_evaluations_by_ply[ply]++;
            }
        } else {
            int dynamic[3] = {0, 0, 0};
            const int *dynamic_features = NULL;
    ...
    ctx->ordering_nodes_by_ply[ply]++;
    ctx->ordering_actions_by_ply[ply] += legal_count;
    for (size_t i = 1; i < legal_count; i++) {
        GCSemanticOrderedAction item = ordered[i];
        size_t j = i;
        while (j > 0) {
            GCSemanticOrderedAction previous = ordered[j - 1];
            if (previous.mover_score > item.mover_score ||
                (previous.mover_score == item.mover_score &&
    ...
    for (size_t i = 0; i < legal_count; i++)
        actions->data[i] = ordered[i].action;
    actions->count = legal_count;
    free(ordered);
    uint64_t elapsed = gc_monotonic_ns() - start_ns;
...
static int gc_semantic_iterative_negamax(GCSemanticIterativeContext *ctx,
    ...
    int tt_has_action = 0;
    uint64_t declaration_action = 0;
    int declaration_win = gc_semantic_declaration_win(
        ctx->rules, position, ctx->root_ply_offset + ply,
        &declaration_action);
    ...
    if (declaration_win > 0) {
        ...
        ctx->pv_table[(size_t)ply * ctx->pv_stride] = declaration_action;
        ctx->pv_length[ply] = 1;
    ...
    if (ply == 0 && ctx->root_order_hint_present) {
        for (size_t j = 0; j < actions.count; j++) {
            if (actions.data[j] == ctx->root_order_hint) {
                ctx->root_hint_legal = 1;
                uint64_t first = actions.data[0];
                actions.data[0] = actions.data[j];
                actions.data[j] = first;
                break;
...
static int gc_semantic_iterative_fallback(GCSemanticIterativeContext *ctx,
    ...
    uint64_t declaration_action = 0;
    int declaration_win = gc_semantic_declaration_win(
        ctx->rules, &ctx->stack[0], ctx->root_ply_offset,
        &declaration_action);
    if (declaration_win < 0) {
        ctx->control = 4;
        return 0;
...
static int gc_semantic_parse_profile(PyObject *board_values,
                                     ...
                                     GCSemanticProbeProfile *profile) {
    memset(profile, 0, sizeof(*profile));
    if (evaluator_scale < 1 || evaluator_scale > 1024) {
        PyErr_SetString(PyExc_ValueError,
                        "semantic evaluator scale must be in [1, 1024]");
        return 0;
    }
    profile->evaluator_scale = (int)evaluator_scale;
    ...
    if (spatial_values != NULL) {
        Py_ssize_t expected = (Py_ssize_t)2 * rules->type_count * GC_SEMANTIC_SPATIAL_CELLS;
        if ((!PyList_Check(spatial_values) && !PyTuple_Check(spatial_values)) ||
            PySequence_Size(spatial_values) != expected) {
            PyErr_SetString(PyExc_ValueError,
        ...
        for (uint8_t owner = 0; owner < 2; owner++) {
            for (uint16_t type = 0; type < rules->type_count; type++) {
                long long sum = 0;
                for (int cell = 0; cell < GC_SEMANTIC_SPATIAL_CELLS; cell++) {
                    Py_ssize_t index = ((Py_ssize_t)owner * rules->type_count + type) *
                        GC_SEMANTIC_SPATIAL_CELLS + cell;
                    PyObject *value = PySequence_GetItem(spatial_values, index);
                    long v = PyLong_AsLong(value);
                    Py_DECREF(value);
                    if (PyErr_Occurred() || v < -1000000 || v > 1000000) {
                        PyErr_SetString(PyExc_ValueError,
                                        "semantic spatial evaluator values must be bounded integers");
                        return 0;
                    }
                    profile->spatial[owner][type][cell] = (int)v;
                    sum += v;
                }
                if (sum != 0) {
                    PyErr_SetString(PyExc_ValueError,
                                    "semantic spatial evaluator rows must have zero sum");
                    return 0;
                }
            }
        }
        profile->spatial_supplied = 1;
    }
    if (localized_control_values != NULL) {
        if ((!PyList_Check(localized_control_values) && !PyTuple_Check(localized_control_values)) ||
            PySequence_Size(localized_control_values) != GC_SEMANTIC_SPATIAL_CELLS) {
            PyErr_SetString(PyExc_ValueError,
                            "semantic localized control profile must contain nine values");
            return 0;
        }
        long long sum = 0;
        for (int cell = 0; cell < GC_SEMANTIC_SPATIAL_CELLS; cell++) {
            PyObject *value = PySequence_GetItem(localized_control_values, cell);
            long v = PyLong_AsLong(value);
            Py_DECREF(value);
            if (PyErr_Occurred() || v < -1000000 || v > 1000000) {
                PyErr_SetString(PyExc_ValueError,
                                "semantic localized control values must be bounded integers");
                return 0;
            }
            profile->localized_control[cell] = (int)v;
            sum += v;
        }
        if (sum != 0) {
            PyErr_SetString(PyExc_ValueError,
                            "semantic localized control values must have zero sum");
            return 0;
        }
...
static PyObject *gc_semantic_evaluate(PyObject *self, PyObject *args) {
    (void)self;
    PyObject *rules_capsule, *position_capsule;
...
static PyObject *gc_semantic_iterative_search(
    ...
    PyObject *ordering_board_values = Py_None;
    PyObject *ordering_hand_values = Py_None;
    PyObject *ordering_dynamic_values = Py_None;
    ...
    if (!PyArg_ParseTuple(args, "OOI|OOOOOOOOOIIOIOOOOOOOOIiiO", &rules_capsule, &position_capsule,
                          ...
                          &root_window_pruning_obj,
                          &ordering_board_values, &ordering_hand_values,
    ...
    uint64_t root_order_hint = 0;
    int root_order_hint_present = root_order_hint_obj != Py_None;
    ...
    if (!gc_semantic_parse_profile(ordering_board_values, ordering_hand_values,
                                   ordering_dynamic_values, ordering_spatial_values,
                                   ordering_localized_control_values,
                                   ordering_compact_values, rules,
                                   ordering_evaluator_scale, &ordering_profile)) {
        gc_semantic_profile_free(&profile);
        return NULL;
    }
    ...
    if ((learned_move_ordering || policy_move_ordering) && root_order_hint_present) {
        ...
        gc_semantic_profile_free(&ordering_profile);
        PyErr_SetString(PyExc_ValueError,
                        "semantic learned move ordering cannot combine with root_order_hint");
        return NULL;
    }
    GCCancelFlag *cancel = NULL;
    if (cancel_capsule != Py_None) {
    ...
    GCSemanticTable *borrowed_tt = NULL;
    ...
    for (uint32_t depth = 1; depth <= max_depth; depth++) {
        if (!gc_semantic_iterative_check_budget(&ctx, 1)) break;
        ctx.root_iteration_attempted[depth] = 1;
    ...
    int completed_is_declaration = completed_has_action &&
        gc_semantic_is_declaration_action(completed_action);
    ...
    PyObject *ordering_nodes_by_ply = gc_semantic_u64_tuple(
        ctx.ordering_nodes_by_ply, GC_SEM_MAX_PLY + 1);
    PyObject *ordering_actions_by_ply = gc_semantic_u64_tuple(
        ctx.ordering_actions_by_ply, GC_SEM_MAX_PLY + 1);
    PyObject *ordering_elapsed_by_ply = gc_semantic_u64_tuple(
        ctx.ordering_elapsed_nanoseconds_by_ply, GC_SEM_MAX_PLY + 1);
    PyObject *ordering_cache_hits_by_ply = gc_semantic_u64_tuple(
        ctx.ordering_cache_hits_by_ply, GC_SEM_MAX_PLY + 1);
    PyObject *ordering_cache_misses_by_ply = gc_semantic_u64_tuple(
    ...
    if (!ordering_max_ply_obj || !ordering_evaluations_by_ply ||
        ...
        PyDict_SetItemString(out, "ordering_actions_by_ply", ordering_actions_by_ply) != 0 ||
        ...
        Py_XDECREF(ordering_actions_by_ply);
        Py_XDECREF(ordering_elapsed_by_ply);
    ...
    Py_DECREF(ordering_max_ply_obj);
    Py_DECREF(ordering_evaluations_by_ply);
    Py_DECREF(ordering_nodes_by_ply);
    Py_DECREF(ordering_actions_by_ply);
    Py_DECREF(ordering_elapsed_by_ply);
    ...
    for (uint32_t depth = 1; depth <= max_depth; depth++)
        attempted_count += ctx.root_iteration_attempted[depth] ? 1ul : 0ul;
    PyObject *root_first_actions = PyTuple_New((Py_ssize_t)attempted_count);
    if (!root_hint_requested || !root_first_actions) {
        Py_XDECREF(root_hint_requested);
        Py_XDECREF(root_first_actions);
        Py_DECREF(out);
        Py_DECREF(best_action_obj);
        Py_DECREF(pv);
        free(ctx.stack); free(ctx.pv_table); free(ctx.pv_length);
    ...
    unsigned long attempted_index = 0;
    for (uint32_t depth = 1; depth <= max_depth; depth++) {
        if (!ctx.root_iteration_attempted[depth]) continue;
        PyObject *first = ctx.root_first_action_present[depth]
            ? PyLong_FromUnsignedLongLong(ctx.root_first_actions[depth])
            : Py_NewRef(Py_None);
        if (!first) {
            Py_DECREF(root_hint_requested);
        ...
        PyTuple_SET_ITEM(root_first_actions, (Py_ssize_t)attempted_index++, first);
    }
    PyObject *root_hint_apply_count = PyLong_FromUnsignedLongLong(
        ctx.root_hint_apply_count);
    PyObject *root_iterations_attempted = PyLong_FromUnsignedLong(attempted_count);
    if (!root_hint_apply_count || !root_iterations_attempted ||
        PyDict_SetItemString(out, "root_hint_requested", root_hint_requested) != 0 ||
        ...
        PyDict_SetItemString(out, "learned_move_ordering",
                             ctx.learned_move_ordering ? Py_True : Py_False) != 0 ||
        gc_semantic_set_u64(out, "ordering_evaluations", ctx.ordering_evaluations) != 0 ||
        gc_semantic_set_u64(out, "ordering_nodes", ctx.ordering_nodes) != 0 ||
        gc_semantic_set_u64(out, "ordering_actions", ctx.ordering_actions) != 0 ||
        gc_semantic_set_u64(out, "ordering_elapsed_nanoseconds",
                            ctx.ordering_elapsed_nanoseconds) != 0) {
    ...
    Py_DECREF(root_hint_apply_count);
    Py_DECREF(root_iterations_attempted);
    Py_DECREF(root_hint_requested);
    Py_DECREF(root_first_actions);
    Py_DECREF(best_action_obj);
    Py_DECREF(pv);
    ...
    if (completed_is_declaration) {
        ...
        if (declaration_index >= rules->declaration_count) {
            Py_DECREF(out);
            PyErr_SetString(PyExc_RuntimeError,
                            "semantic search returned an invalid declaration tag");
            return NULL;
        }
        PyObject *declaration_id = PyUnicode_FromString(
            rules->declarations[declaration_index].declaration_id);
        if (declaration_id == NULL || PyDict_SetItemString(
                out, "declaration_id", declaration_id) != 0) {
            Py_XDECREF(declaration_id);
            Py_DECREF(out);
            return NULL;
        }
        Py_DECREF(declaration_id);
    } else if (PyDict_SetItemString(out, "declaration_id", Py_None) != 0) {
        Py_DECREF(out);
        return NULL;
    }
    return out;
...
static GCSemanticSearchEngine *gc_get_semantic_engine(PyObject *capsule) {
    if (!PyCapsule_CheckExact(capsule)) {
        PyErr_SetString(PyExc_TypeError, "expected a native semantic engine capsule");
        return NULL;
    }
    return (GCSemanticSearchEngine *)PyCapsule_GetPointer(
        capsule, GC_SEM_ENGINE_CAPSULE);
}
...
static PyObject *gc_create_semantic_search_engine(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "OOOOOOOI|IOOOOOOIppiiO", &rules_capsule, &board_values,
                          ...
                          &ordering_hand_values, &ordering_dynamic_values,
    ...
    if (tt_megabytes > 1024) {
        PyErr_SetString(PyExc_ValueError,
                        "semantic engine tt_megabytes must be in [0, 1024]");
        return NULL;
    }
    if (ordering_max_ply < -1 || ordering_max_ply > GC_SEM_MAX_PLY) {
        PyErr_SetString(PyExc_ValueError,
    ...
    engine->ordering_dynamic_values = Py_NewRef(ordering_dynamic_values);
    engine->ordering_spatial_values = Py_NewRef(ordering_spatial_values);
    engine->ordering_localized_control_values = Py_NewRef(ordering_localized_control_values);
    engine->ordering_compact_values = Py_NewRef(ordering_compact_values);
    engine->ordering_evaluator_scale = ordering_evaluator_scale;
    if (tt_megabytes != 0) {
...
static PyObject *gc_semantic_engine_search(PyObject *self, PyObject *args) {
    (void)self;
    PyObject *engine_capsule, *position_capsule;
    unsigned int max_depth;
    ...
    if (engine->busy) {
        PyErr_SetString(PyExc_RuntimeError, "semantic search engine is busy");
        return NULL;
    }
    PyObject *tt_capsule = engine->tt != NULL
    ...
    Py_INCREF(position_capsule);
    PyTuple_SET_ITEM(call_args, 1, position_capsule);
    PyTuple_SET_ITEM(call_args, 2, PyLong_FromUnsignedLong(max_depth));
    Py_INCREF(max_nodes); PyTuple_SET_ITEM(call_args, 3, max_nodes);
    Py_INCREF(max_time); PyTuple_SET_ITEM(call_args, 4, max_time);
    ...
    Py_INCREF(engine->ordering_board_values); PyTuple_SET_ITEM(call_args, 18, engine->ordering_board_values);
    Py_INCREF(engine->ordering_hand_values); PyTuple_SET_ITEM(call_args, 19, engine->ordering_hand_values);
    Py_INCREF(engine->ordering_dynamic_values); PyTuple_SET_ITEM(call_args, 20, engine->ordering_dynamic_values);
    Py_INCREF(engine->ordering_spatial_values); PyTuple_SET_ITEM(call_args, 21, engine->ordering_spatial_values);
    Py_INCREF(engine->ordering_localized_control_values); PyTuple_SET_ITEM(call_args, 22, engine->ordering_localized_control_values);
...
static PyObject *gc_semantic_engine_clear_tt(PyObject *self, PyObject *args) {
    (void)self;
    PyObject *engine_capsule;
    if (!PyArg_ParseTuple(args, "O", &engine_capsule)) return NULL;
    GCSemanticSearchEngine *engine = gc_get_semantic_engine(engine_capsule);
    if (engine == NULL) return NULL;
    if (engine->busy) {
        PyErr_SetString(PyExc_RuntimeError, "semantic search engine is busy");
    ...
    gc_semantic_tt_clear(engine->tt);
    Py_RETURN_NONE;
}

static PyObject *gc_semantic_engine_tt_info(PyObject *self, PyObject *args) {
    (void)self;
    PyObject *engine_capsule;
    if (!PyArg_ParseTuple(args, "O", &engine_capsule)) return NULL;
    GCSemanticSearchEngine *engine = gc_get_semantic_engine(engine_capsule);
    ...
    if (engine->tt == NULL) {
        return Py_BuildValue("{s:K,s:K,s:K,s:K,s:K,s:K,s:K}",
            "requested_bytes", (uint64_t)0, "allocated_bytes", (uint64_t)0,
            "bucket_count", (uint64_t)0, "entry_capacity", (uint64_t)0,
            "entry_size", (uint64_t)gc_semantic_tt_entry_bytes(),
            "generation", (uint64_t)0, "occupied_entries", (uint64_t)0);
    }
    return Py_BuildValue("{s:K,s:K,s:K,s:K,s:K,s:K,s:K}",
        "requested_bytes", (uint64_t)engine->tt->requested_bytes,
        "allocated_bytes", (uint64_t)engine->tt->allocated_bytes,
        "bucket_count", (uint64_t)engine->tt->bucket_count,
        "entry_capacity", (uint64_t)(engine->tt->bucket_count * GC_SEM_TT_WAYS),
        "entry_size", (uint64_t)gc_semantic_tt_entry_bytes(),
        "generation", (uint64_t)engine->tt->generation,
        "occupied_entries", (uint64_t)engine->tt->occupied_entries);
}
...
static PyMethodDef gc_methods[] = {
    ...
    {"semantic_spatial_features", gc_semantic_spatial_features, METH_VARARGS,
     "semantic_spatial_features(rules, position) -> occupancy and localized control"},
    {"semantic_evaluate", gc_semantic_evaluate, METH_VARARGS,
     "semantic_evaluate(rules, position, profiles...) -> fixed-point leaf score"},
    ...
    {"semantic_engine_clear_tt", gc_semantic_engine_clear_tt, METH_VARARGS,
     "semantic_engine_clear_tt(engine) -> None"},
    {"semantic_engine_tt_info", gc_semantic_engine_tt_info, METH_VARARGS,
     "semantic_engine_tt_info(engine) -> dict"},
