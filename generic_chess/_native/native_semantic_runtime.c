...
int gc_semantic_runtime_delta_make_trusted(GCSemanticPosition *position, const GCSemanticRules *r, uint64_t action, GCSemanticDeltaUndo *undo) {
    ...
    if (!postconditions_hold(r, position, pattern, side, target)) goto fail;
    ...
    position->history_lo[position->history_len] = words[0]; position->history_hi[position->history_len] = words[1]; memcpy(position->history_digest[position->history_len], words, sizeof(words)); position->history_exact = position->history_exact; position->history_len++;
    return 1;
fail:
...
GCSemanticDeltaRuntimeStack *gc_semantic_delta_runtime_new(const GCSemanticRules *rules, const GCSemanticPosition *position) {
    ...
    runtime->rules = rules; runtime->current = *position; return runtime;
}
...
int gc_semantic_delta_runtime_push(GCSemanticDeltaRuntimeStack *runtime, uint64_t action) {
    ...
    if (!gc_semantic_runtime_delta_make_trusted(&runtime->current, runtime->rules, action, &runtime->undos[runtime->depth])) return 0;
