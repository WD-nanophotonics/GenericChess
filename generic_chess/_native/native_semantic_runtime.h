...
typedef enum {
    GC_SEM_HISTORY_EXACT_APPEND = 0,
    GC_SEM_HISTORY_TRANSIENT_NONE = 1,
} GCSemanticHistoryPolicy;
...
typedef struct {
    ...
    uint8_t history_policy;
} GCSemanticDeltaRuntimeStack;

GCSemanticDeltaRuntimeStack *gc_semantic_delta_runtime_new(
    const GCSemanticRules *rules,
    const GCSemanticPosition *position);
GCSemanticDeltaRuntimeStack *gc_semantic_delta_runtime_new_transient(
    const GCSemanticRules *rules,
    const GCSemanticPosition *position);
