...
ROUND = ROOT / "artifacts" / "round5_alphasho_benchmark_corrective_r1"
...
def _write_historical_manifest(output: Path, name: str) -> dict[str, Any]:
    value = _history_manifest(HISTORICAL)
...
def _calibrate(output: Path, compiled, worker, suite, legacy_eval, generic) -> dict[str, Any]:
    rows = []
    profiles = (("gc_legacy", "gc", legacy_eval), ("alphasho_legacy", "alphasho", "legacy"),
                ("gc_generic", "gc", generic), ("alphasho_current", "alphasho", "current"))
        ...
        for opening in suite[:10]:
            for color in ("black", "white"):
                for name, engine, evaluator_or_profile in profiles:
                    ...
                    started = time.monotonic()
                    if engine == "gc":
                        decision = R1GCPlayer(compiled, evaluator_or_profile).choose(session, "seconds", budget)
                    else:
                        result = worker.request({"command": "choose", "profile": evaluator_or_profile,
                                                 "budget_kind": "seconds", "budget": budget,
                                                 "sfen": opening["sfen"], "initial_sfen": opening["sfen"], "history": []})
                        decision = result.get("search_info") or {}
                    wall = time.monotonic() - started
                    elapsed = float(decision.get("elapsed_seconds", wall))
                    rows.append({"budget": budget, "opening": opening["name"], "color": color,
                                 "profile": name, "elapsed_seconds": elapsed, "wall_seconds": wall,
                                 "timing_valid": elapsed <= budget + max(0.050, budget * 0.05)})
    _write_jsonl(output / "runtime_calibration.jsonl", rows)
    summary = {}
    for budget in WALL_BUDGETS:
        selected = [r for r in rows if r["budget"] == budget]
        summary[str(budget)] = {"budget": budget, "trials": len(selected),
                                "timing_invalid": sum(not r["timing_valid"] for r in selected),
                                "max_elapsed_seconds": max((r["elapsed_seconds"] for r in selected), default=None),
                                "median_elapsed_seconds": statistics.median([r["elapsed_seconds"] for r in selected]) if selected else None}
    qualified = [b for b in WALL_BUDGETS if summary[str(b)]["timing_invalid"] == 0]
    if len(qualified) < 2:
        raise RuntimeError({"kind": "R1_TIMING_CALIBRATION", "status": "FAIL_INSUFFICIENT_VALID_BUDGETS", "summary": summary})
    freeze = {"schema_version": 1, "candidate_budgets_seconds": list(WALL_BUDGETS), "qualified_budgets_seconds": qualified,
              "LOW_SECONDS": qualified[0], "HIGH_SECONDS": qualified[1], "selection_inputs_runtime_only": True,
              "profiles": [p[0] for p in profiles], "timing_tolerance": TIMING_TOLERANCE,
...
def run(output: Path = ROUND) -> None:
    ...
    if not clean or not checkpoint_ancestor or not baseline_ancestor:
    ...
    suite = _load_suite()
    _write(output / "baseline.json", {"checkpoint_sha": CHECKPOINT_SHA, "baseline_ancestor": BASELINE_SHA,
    ...
    _write(output / "harness_provenance.json", {"mode": "R1", "harness_sha_required": harness_sha,
                                                 "branch": "sandbox", "legal_rule_monkey_patch": False,
                                                 "lossless_semantic_action_mapping": True})
    _write_historical_manifest(output, "legacy_round5_artifact_tree_before.json")
    ...
    if before != after:
        raise RuntimeError("AlphaSho repository changed during R1")
    old_after = _write_historical_manifest(output, "legacy_round5_artifact_tree_after.json")
    old_before = json.loads((output / "legacy_round5_artifact_tree_before.json").read_text(encoding="utf-8"))
    if old_before != old_after:
        raise RuntimeError("historical Round 5 artifact tree changed")
    ...
    gates = {
        ...
        "LEGACY_ROUND5_ARTIFACT_TREE_IMMUTABLE": old_before == old_after,
    ...
    _write(output / "final_verdict.json", {"ROUND5_CORRECTIVE_R1": "PASS", "R1_HARNESS_SHA": harness_sha,
